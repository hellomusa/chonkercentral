...
export async function checkAvailability(dateTime: string): Promise<Map<number, boolean>> {
  ...
  console.log("Fetching LibCal availability for date:", today, " ending ", tomorrow);
  
  const response = await fetch('https://carletonu.libcal.com/spaces/availability/grid', {
    method: 'POST',
    headers: {
